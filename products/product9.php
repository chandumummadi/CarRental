...
try {
  ...
  $incrementViewCount = "UPDATE tblvehicles SET ViewCount = ViewCount + 1 WHERE id = :vhid";
  $query = $dbh->prepare($incrementViewCount);
...
// Remove product if it already exists and add it to the start
$recentProducts = array_diff($recentProducts, [$vhid]);
array_unshift($recentProducts, $vhid);

// Keep only the last 5 products
$recentProducts = array_slice($recentProducts, 0, 5);

// Update the cookie
setcookie('recent_products', json_encode($recentProducts), time() + (86400 * 30), "/"); // 30 days expiration
...
if (isset($_POST['submit_review'])) {
  ...
  if ($user_id && $rating > 0 && $rating <= 5) {
      $sql = "INSERT INTO tblreviews (product_id, user_id, rating, review) VALUES (:product_id, :user_id, :rating, :review)";
      $query = $dbh->prepare($sql);
      $query->bindParam(':product_id', $vhid, PDO::PARAM_INT);
...
$sql = "SELECT tblusers.FullName, tblreviews.rating, tblreviews.review, tblreviews.created_at 
      FROM tblreviews 
      JOIN tblusers ON tblreviews.user_id = tblusers.id 
      WHERE tblreviews.product_id = :product_id";
$query = $dbh->prepare($sql);
$query->bindParam(':product_id', $vhid, PDO::PARAM_INT);
$query->execute();
...
<a href="#loginform" class="btn btn-xs uppercase" data-toggle="modal" data-dismiss="modal">Login For Book</a>
    ...
    <div class="reviews-section">
            ...
            <ul class="list-group">
                <?php foreach ($reviews as $review) { ?>
                    <li class="list-group-item">
                        <strong><?php echo htmlentities($review->FullName); ?></strong> rated 
                        <strong><?php echo htmlentities($review->rating); ?>/5</strong>
                        <p><?php echo htmlentities($review->review); ?></p>
                        <small>Reviewed on <?php echo htmlentities($review->created_at); ?></small>
                    </li>
                <?php } ?>
            </ul>
        <?php } else { ?>
        ...
        <?php } else { ?>
            <p>Please <a href="../includes/login.php">login</a> to leave a review.</p>
        <?php } ?>
            </div>
